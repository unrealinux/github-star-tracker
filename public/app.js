...
const setApiKey = (k) =>
  k ? localStorage.setItem("gst_api_key", k) : localStorage.removeItem("gst_api_key");
const getSession = () => localStorage.getItem("gst_session") || "";
const setSession = (t) =>
  t ? localStorage.setItem("gst_session", t) : localStorage.removeItem("gst_session");
...
function renderStats(stats) {
  ...
  const TOKEN_VIEW = {
    ...
    invalid: {
      text: "Token 失效·已回退匿名",
      cls: "warn",
      title: "GitHub 返回 401，已自动改用匿名请求（配额较低）。请更换 GITHUB_TOKEN 或 GH_TOKEN",
    },
  ...
  if (gq?.search)
    $("quota-badge").title =
      `GitHub 搜索配额剩余 ${gq.search.remaining}/${gq.search.limit}（低于保护下限时自动刷新会跳过）`;
}
...
async function loadWebhookTypes() {
  ...
  try {
    ...
    sel.innerHTML = (types || [])
      .map((t) => `<option value="${t}">${escapeHtml(labels[t] || t)}</option>`)
      .join("");
...
async function loadSettings() {
  try {
    ...
    if ($("api-key-status"))
      $("api-key-status").textContent = s.authEnabled ? "（服务端已启用认证）" : "（服务端未启用认证）";
...
function populateLanguages() {
  ...
  sel.innerHTML =
    ...
    languages
      .map((l) => `<option value="${escapeHtml(l.language)}">${escapeHtml(l.language)}（${l.c}）</option>`)
      .join("");
...
function lineChart({ series, width = 680, height = 220, logScale = false, yFormat = fmt, markers = [] }) {
  ...
  const grid = gridY
    .map(
      (gy) =>
        `<line x1="${padL}" y1="${gy.toFixed(1)}" x2="${padL + iw}" y2="${gy.toFixed(1)}" class="grid"/>`,
    )
    .join("");
  const yTicks = gridY
    .map(
      (gy, i) =>
        `<text x="${padL - 6}" y="${(gy + 3).toFixed(1)}" text-anchor="end" class="axis">${escapeHtml(yFormat(gridVals[i]))}</text>`,
    )
    .join("");
  ...
  const markerSvg = (markers || [])
    .filter((m) => idx.has(m.x))
    .map((m) => {
      const mx = x(idx.get(m.x));
      const c = m.color || "#f59e0b";
      return (
        `<line x1="${mx.toFixed(1)}" y1="${padT}" x2="${mx.toFixed(1)}" y2="${padT + ih}" stroke="${c}" stroke-width="1" stroke-dasharray="3 3" opacity="0.7"/>` +
        (m.label
          ? `<text x="${(mx + 3).toFixed(1)}" y="${padT + 10}" class="axis" fill="${c}">${escapeHtml(m.label)}</text>`
          : "")
      );
    })
    .join("");
  ...
  const lines = series
    .map((s, si) => {
      ...
      const pts = s.points
        .filter((p) => idx.has(p.x))
        .map((p) => `${x(idx.get(p.x)).toFixed(1)},${y(p.y).toFixed(1)}`)
        .join(" ");
      const dots = s.points
        .filter((p) => idx.has(p.x))
        .map((p) => {
          const cx = x(idx.get(p.x)).toFixed(1),
            cy = y(p.y).toFixed(1);
          return (
            `<circle cx="${cx}" cy="${cy}" r="8" fill="transparent"><title>${escapeHtml(s.name)} · ${escapeHtml(p.x)}: ${escapeHtml(yFormat(p.y))}</title></circle>` +
  ...
  const legend = series
    .map(
      (s, si) =>
        `<span class="legend-item"><i style="background:${s.color || CHART_COLORS[si % CHART_COLORS.length]}"></i>${escapeHtml(s.name)}</span>`,
    )
    .join("");
  ...
  (root || document).querySelectorAll("svg.line-chart[data-chart-id]").forEach((svg) => {
    ...
    svg.addEventListener("mousemove", (e) => {
      ...
      const rows = cfg.series
        ...
        .join("");
      const markRows = (cfg.markers || [])
        .filter((m) => m.x === label)
        .map(
          (m) =>
            `<div class="tip-row tip-marker" style="color:${m.color || "#f59e0b"}">✦ <span>${escapeHtml(m.message || m.label || "")}</span></div>`,
        )
        .join("");
...
function renderRepos() {
  ...
  if (state.total === 0) {
    el.innerHTML =
      '<div class="empty"><p>没有符合条件的项目</p><p class="muted">可降低筛选阈值，或点击「立即更新数据」。</p></div>';
    renderPagination();
    return;
  }
...
async function openDetail(id) {
  try {
    const data = await fetchJson(
      `/api/repos/${id}/history?metric=${encodeURIComponent(state.metric || "stars")}`,
    );
    const r = data.repo;
    ...
    const drawHistory = () => {
      ...
      if (pts.length < 2) {
        $("hist-chart").innerHTML = '<p class="muted">快照不足，继续积累后可查看趋势。</p>';
        return;
      }
    ...
    $("hist-controls")
      ...
      .forEach((b) =>
        b.addEventListener("click", () => {
          ...
          $("hist-controls")
            .querySelectorAll("button[data-range]")
            .forEach((x) => (x.className = `btn btn-sm ${x === b ? "btn-primary" : "btn-secondary"}`));
          drawHistory();
        }),
      );
    ...
    $("repo-threshold-save").addEventListener("click", async () => {
      ...
      try {
        ...
        $("repo-threshold-hint").textContent =
          res.threshold == null ? "已恢复全局阈值" : `已设为 ${res.threshold}`;
      } catch (err) {
    ...
    // C4: 相似项目
    try {
      const sim = await fetchJson(`/api/repos/${r.id}/similar?limit=6`);
      $("similar-block").innerHTML = sim.similar.length
        ? sim.similar
            .map(
              (x) => `<div class="similar-item" data-id="${x.id}">
    ...
    try {
      ...
      $("enrich-block").innerHTML =
        `${chips ? `<div class="topic-row">${chips}</div>` : ""}<div class="fact-row">${facts}</div>${rel}`;
    } catch (err) {
      $("enrich-block").innerHTML =
        `<p class="muted" style="font-size:12px">获取失败：${escapeHtml(err.message)}</p>`;
    }
  } catch (e) {
    showError(e.message);
  }
...
async function loadFavorites() {
  try {
    ...
    if (repos.length === 0) {
      el.innerHTML =
        '<div class="empty"><p>还没有收藏任何仓库</p><p class="muted">点击仓库行查看详情，在弹窗中点「☆ 收藏」</p></div>';
    ...
    renderPager("fav-pagination", data, (p) => {
      state.favPage = p;
      loadFavorites();
    });
  } catch (e) {
...
async function loadCustomRepos() {
  try {
    ...
    if (repos.length === 0) {
      el.innerHTML =
        '<div class="empty"><p>还没有添加自定义仓库</p><p class="muted">点击右上角「+ 添加仓库」开始追踪</p></div>';
...
async function loadLanguageTrends() {
  try {
    ...
    const el = $("trend-list");
    if (trends.length === 0) {
      el.innerHTML = '<div class="empty"><p>暂无数据，先运行几次抓取</p></div>';
      return;
    }
...
async function loadLeaderboard() {
  try {
    ...
    starsEl.innerHTML =
      (data.byStars || []).map((r, i) => lbItem(r, i + 1, "stars")).join("") ||
      '<div class="empty"><p>暂无数据</p></div>';
    growthEl.innerHTML =
      (data.byGrowth || []).map((r, i) => lbItem(r, i + 1, "growth")).join("") ||
      '<div class="empty"><p>尚无足够的增长数据（需至少 2 条快照）</p></div>';
...
async function loadSurges() {
  try {
    ...
    const data = await fetchJson(
      `/api/trends/surge?limit=${limit}&minStars=${minStars}&decaying=1&metric=${metric}`,
    );
...
async function runReplay() {
  ...
  try {
    const d = await fetchJson(
      `/api/replay?at=${encodeURIComponent(at)}&window=${window}&sort=${sort}&limit=50`,
    );
    ...
    $("replay-result").innerHTML =
      ...
      <div class="repo-row" data-id="${r.id}">
        ...
        .join("") || '<div class="empty"><p>该时刻无数据</p></div>';
...
async function loadExternalMetrics() {
  try {
    if (!SOURCES.length) {
      ...
      if (sel)
        sel.innerHTML = SOURCES.map((x) => `<option value="${x.key}">${escapeHtml(x.label)}</option>`).join(
          "",
        );
    }
    ...
    if (!metrics.length) {
      el.innerHTML =
        '<div class="empty"><p>还没有追踪任何外部指标</p><p class="muted">点击右上角「+ 添加指标」，例如 npm 的 react</p></div>';
      return;
    }
    el.innerHTML = metrics.map((r, i) => extItem(r, i + 1)).join("");
    el.querySelectorAll(".ext-del").forEach((btn) =>
      btn.addEventListener("click", async () => {
...
function exportCsv() {
  ...
  const csv =
    "\uFEFF" +
    [header, ...rows].map((row) => row.map((c) => `"${String(c).replace(/"/g, '""')}"`).join(",")).join("\n");
...
async function initCompare() {
  if (!cmpReady) {
    try {
      ...
      $("cmp-options").innerHTML = opts
        .map((r) => `<option value="${escapeHtml(r.full_name)}"></option>`)
        .join("");
      cmpReady = true;
    } catch (e) {
      console.error("initCompare:", e);
    }
  }
  renderCompareChips();
  if (cmpSelection.length >= 2) loadCompare();
  else renderCompareEmpty();
}

function renderCompareChips() {
  const el = $("cmp-chips");
  el.innerHTML = cmpSelection
    .map(
      (s) =>
        `<span class="chip">${escapeHtml(s.full_name)}<button class="chip-x" data-id="${s.id}" aria-label="移除">×</button></span>`,
    )
    .join("");
...
async function loadOverview() {
  ...
  try {
    ...
    $("ov-stars").innerHTML =
      d.totalStars.length >= 2
        ? lineChart({
            series: [
              {
                name: "总星数",
                color: "#58a6ff",
                points: d.totalStars.map((p) => ({ x: p.date, y: p.totalStars })),
              },
            ],
            yFormat: compactNum,
          })
    ...
    $("ov-activity").innerHTML = barChart(
      d.snapshotActivity.map((p) => ({ x: p.date, y: p.count })),
      { color: "#22c55e" },
    );
    $("ov-discoveries").innerHTML = barChart(
      d.discoveries.map((p) => ({ x: p.date, y: p.count })),
      { color: "#f59e0b" },
    );
    ...
    for (let i = 1; i < d.totalStars.length; i++) {
      heat.push({
        x: d.totalStars[i].date,
        y: Math.max(0, d.totalStars[i].totalStars - d.totalStars[i - 1].totalStars),
      });
    }
...
async function loadSignals() {
  ...
  try {
    ...
    const [anomaly, rising, events] = await Promise.all([
      fetchJson(
        `/api/trends/anomaly?days=${an.value}&z=${az.value}&minSamples=${am.value}&minStars=${ams.value}&limit=10`,
      ),
      fetchJson(`/api/trends/rising?minStars=${rmin.value}&minDays=${rdays.value}&limit=15${rmaxQ}`),
      fetchJson(
        `/api/trends/events?days=${$("ev-days").value}&z=${$("ev-z").value}&minRepos=${$("ev-min-repos").value}&minSamples=${$("ev-min").value}&limit=10`,
      ),
    ]);
    ...
    document
      .querySelectorAll("#an-spikes .lb-item, #an-drops .lb-item")
      .forEach((el) => el.addEventListener("click", () => openDetail(Number(el.dataset.id))));

    $("rs-notice").className = "lb-notice";
    $("rs-notice").innerHTML =
      `ℹ️ 星数上限为 <b>${fmt(rising.maxStars)}</b>（留空时自动取追踪集星数中位数）· 仅纳入 ≥${rising.minDays} 天前创建的项目 · 按「日增百分比」排序。`;
    $("rs-list").innerHTML = rising.rising.length
      ? rising.rising
          .map(
            (r, i) => `<div class="lb-item" data-id="${r.id}">
    ...
    document
      .querySelectorAll("#rs-list .lb-item")
      .forEach((el) => el.addEventListener("click", () => openDetail(Number(el.dataset.id))));

    // 🌊 事件聚类
    $("ev-notice").innerHTML =
      `ℹ️ 同时将「同一天多个仓库 z 异常」归为一波趋势，用语言/描述共同词自动命名。需至少 <b>${events.minSamples + 1}</b> 天日增数据。`;
    ...
    document
      .querySelectorAll("#ev-list .event-repo")
      .forEach((el) => el.addEventListener("click", () => openDetail(Number(el.dataset.id))));
  } catch (e) {
...
async function runBacktest() {
  ...
  try {
    ...
    $("bt-notice").innerHTML =
      `ℹ️ 过去 <b>${d.days}</b> 天评估了 <b>${s.evaluatedRepos}</b> 个仓库（平均 ${s.avgSampleDays} 天样本）。预计 <b>${s.total}</b> 次触发，约 <b>${perDay} 次/天</b>。${s.total > d.days * 2 ? "阈值偏低，建议调高。" : "频率合理。"}回测与生产共用同一套规则（权限覆盖/里程碑/掉星）。`;

    $("bt-repos").innerHTML = d.topRepos.length
      ? d.topRepos
          .map(
            (r, i) =>
              `<div class="lb-item"><span class="lb-rank">${i + 1}</span><div class="lb-main"><span class="repo-name">${escapeHtml(r.full_name)}</span></div><div class="lb-nums"><div class="lb-value">${r.count} 次</div></div></div>`,
          )
          .join("")
    ...
    $("bt-fires").innerHTML = d.fired.length
      ? d.fired
          ...
          .map(
            (f) =>
              `<div class="lb-item" data-id="${f.id}"><span class="lb-rank">${f.kind === "growth" ? "📈" : f.kind === "drop" ? "📉" : "🏁"}</span><div class="lb-main"><span class="repo-name">${escapeHtml(f.full_name)}</span><div class="lb-sub">${escapeHtml(f.date)} · ${escapeHtml(f.kind)}</div></div><div class="lb-nums"><div class="lb-value" style="color:${f.growth >= 0 ? "var(--green)" : "var(--red)"}">${f.growth >= 0 ? "+" : ""}${fmt(f.growth)}</div></div></div>`,
          )
          .join("")
      : '<div class="empty"><p>无触发</p></div>';
    document
      .querySelectorAll("#bt-fires .lb-item[data-id]")
      .forEach((el) => el.addEventListener("click", () => openDetail(Number(el.dataset.id))));
  } catch (e) {
...
function aggregateChart(containerId, data) {
  ...
  $(containerId).innerHTML = lineChart({
    series: [
      { name: `指数（${data.weight === "cap" ? "星数加权" : "等权"}）`, color: "var(--accent)", points },
    ],
...
function membersTable(data) {
  ...
  return (
    `<table class="data-table"><thead><tr><th>成员</th><th>语言</th><th>当前</th><th>区间变化</th><th>占比</th></tr></thead><tbody>` +
    data.members
      .slice(0, 40)
      .map((m) => {
...
async function loadIndices() {
  try {
    indicesCache = (await fetchJson("/api/indices")).indices || [];
  } catch {
    indicesCache = [];
  }
  const el = $("idx-list");
  el.innerHTML = indicesCache.length
    ? indicesCache
        .map(
          (x) => `<div class="entity-row${x.id === currentIndexId ? " active" : ""}" data-id="${x.id}">
...
async function selectIndex(id) {
  currentIndexId = id;
  document
    .querySelectorAll("#idx-list .entity-row")
    .forEach((r) => r.classList.toggle("active", Number(r.dataset.id) === id));
  try {
    const d = await fetchJson(
      `/api/indices/${id}/series?days=${$("idx-days").value}&weight=${$("idx-weight").value}`,
    );
    $("idx-detail-title").textContent =
      `${d.name} · ${d.memberCount} 个成员 · 区间 ${d.changePct >= 0 ? "+" : ""}${d.changePct}%`;
...
async function loadQueries() {
  try {
    queriesCache = (await fetchJson("/api/queries")).queries || [];
  } catch {
    queriesCache = [];
  }
  const el = $("q-list");
...
async function selectQuery(id) {
  currentQueryId = id;
  document
    .querySelectorAll("#q-list .entity-row")
    .forEach((r) => r.classList.toggle("active", Number(r.dataset.id) === id));
  try {
    const d = await fetchJson(`/api/queries/${id}?days=${$("q-days").value}&weight=${$("q-weight").value}`);
    $("q-detail-title").textContent =
      `${d.label} · ${d.memberCount} 个成员 · 区间 ${d.changePct == null ? "—" : (d.changePct >= 0 ? "+" : "") + d.changePct + "%"}`;
...
async function loadViews() {
  try {
    savedViews = (await fetchJson("/api/views")).views || [];
  } catch {
    savedViews = [];
  }
...
on("metric", "change", (e) => {
  ...
  if (state.metric !== "stars" && state.sort === "stars") {
    state.sort = "metric";
    $("sort").value = "metric";
  }
  loadRepos();
...
on("highlight", "input", (e) => {
  state.highlightThreshold = Number(e.target.value) || 0;
  renderRepos();
});
on("pin-hot", "change", (e) => {
  state.pinHot = e.target.checked;
  renderRepos();
});
...
on("clear-alerts", "click", async () => {
  await fetchJson("/api/alerts/read", { method: "POST" });
  loadAlerts();
});
...
on("custom-overlay", "click", (e) => {
  if (e.target === $("custom-overlay")) $("custom-overlay").classList.add("hidden");
});
on("custom-add", "click", async () => {
  ...
  try {
    await fetchJson("/api/custom-repos", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ full_name: fullName, note }),
    });
...
on("test-webhook", "click", async (e) => {
  ...
  btn.disabled = true;
  try {
    const r = await fetchJson("/api/notify/test", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
on("export-db", "click", async () => {
  ...
  try {
    await downloadWithAuth("/api/maintenance/export", `github-star-tracker-backup-${stamp}.db`);
  } catch (err) {
    showError(err.message);
  }
});
on("export-json", "click", async () => {
  const stamp = new Date().toISOString().slice(0, 10);
  try {
    await downloadWithAuth("/api/maintenance/export?format=json", `github-star-tracker-export-${stamp}.json`);
  } catch (err) {
    showError(err.message);
  }
});
...
on("metric-overlay", "click", (e) => {
  if (e.target === $("metric-overlay")) $("metric-overlay").classList.add("hidden");
});
...
on("cmp-input", "keydown", (e) => {
  if (e.key === "Enter") addCompareRepo(e.target.value);
});
on("cmp-clear", "click", () => {
  cmpSelection = [];
  renderCompareChips();
...
[
  ...
  "ev-min-repos",
  "ev-min",
].forEach((id) => on(id, "change", loadSignals));
...
async function checkAuth() {
  try {
    authStatus = await (
      await fetch("/api/auth/status", { headers: getSession() ? { "X-Session": getSession() } : {} })
    ).json();
  } catch {
  ...
  if (authStatus.hasUsers && !authStatus.user) {
    showAuth("login");
    return false;
  }
  return true;
  ...
  try {
    ...
    el.querySelectorAll(".del-user").forEach((b) =>
      b.addEventListener("click", async () => {
        ...
        try {
          await fetchJson(`/api/auth/users/${b.dataset.id}`, { method: "DELETE" });
          loadUsers();
        } catch (e) {
          showError(e.message);
        }
...
const paletteActions = () => [
  ...
  { kind: "动作", title: "🧹 清理过期快照", run: () => $("cleanup-btn")?.click() },
  {
    kind: "动作",
    title: "🔎 打开信号面板",
    run: () => document.querySelector(".tab[data-tab=signals]")?.click(),
  },
];

const paletteTabCommands = () =>
...
function openPalette() {
  ...
  renderPalette("");
  input.focus();
  loadPaletteRepos().then(() => {
    if (!$("palette-overlay").classList.contains("hidden")) renderPalette(input.value);
  });
}
...
function renderPalette(q) {
  ...
  el.innerHTML = paletteItems.length
    ? paletteItems
        .map(
          (x, i) => `<div class="palette-item${i === 0 ? " active" : ""}" data-i="${i}">
