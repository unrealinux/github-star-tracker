...
const MIGRATIONS = [
  ...
  () => {
    ...
    if (!hasCol("repos", "open_issues"))
      db.exec("ALTER TABLE repos ADD COLUMN open_issues INTEGER NOT NULL DEFAULT 0");
    if (!hasCol("snapshots", "forks"))
      db.exec("ALTER TABLE snapshots ADD COLUMN forks INTEGER NOT NULL DEFAULT 0");
    if (!hasCol("snapshots", "open_issues"))
      db.exec("ALTER TABLE snapshots ADD COLUMN open_issues INTEGER NOT NULL DEFAULT 0");
  },
  ...
  () => {
    const hasCol = (table, col) =>
      db
        ...
        .all()
        .some((c) => c.name === col);
    if (!hasCol("alerts", "kind"))
      db.exec("ALTER TABLE alerts ADD COLUMN kind TEXT NOT NULL DEFAULT 'growth'");
  ...
  () => {
    ...
    try {
      ...
      if (!hasCol("alerts", "user_id"))
        db.exec("ALTER TABLE alerts ADD COLUMN user_id INTEGER NOT NULL DEFAULT 0");
      if (!hasCol("push_subscriptions", "user_id"))
        db.exec("ALTER TABLE push_subscriptions ADD COLUMN user_id INTEGER NOT NULL DEFAULT 0");
    } finally {
  ...
  () => {
    const hasCol = (table, col) =>
      db
        ...
        .all()
        .some((c) => c.name === col);
    if (!hasCol("repos", "is_custom"))
      db.exec("ALTER TABLE repos ADD COLUMN is_custom INTEGER NOT NULL DEFAULT 0");
  },
];
...
export function listIndices(userId = 0) {
  return db
    .prepare("SELECT id, user_id, name, spec, created_at FROM indices WHERE user_id = ? ORDER BY id DESC")
    .all(userId)
...
export function addIndex(name, spec, userId = 0) {
  ...
  try {
    const r = db
      .prepare("INSERT INTO indices (name, spec, user_id) VALUES (?, ?, ?)")
      .run(clean, JSON.stringify(spec || {}), userId);
...
export function addTrackedQuery(label, query, userId = 0) {
  ...
  try {
    const r = db
      .prepare("INSERT INTO tracked_queries (label, query, user_id) VALUES (?, ?, ?)")
      .run(l, q, userId);
    return { ok: true, id: Number(r.lastInsertRowid) };
  } catch {
...
export function replaceQueryMembers(queryId, repoIds, ranAt) {
  ...
  try {
    ...
    db.prepare("UPDATE tracked_queries SET member_count = ?, last_run_at = ? WHERE id = ?").run(
      repoIds.length,
...
export function getQueryMemberIds(queryId) {
  return db
    .prepare("SELECT repo_id FROM query_members WHERE query_id = ?")
    .all(queryId)
    .map((r) => r.repo_id);
}
...
export function listPushSubscriptions(userId = null) {
  const sql =
    "SELECT id, user_id, endpoint, p256dh, auth, user_agent, created_at, last_ok_at, last_error FROM push_subscriptions";
...
export function createSession({ token, userId, expiresAt, userAgent }) {
  db.prepare("INSERT INTO sessions (token, user_id, expires_at, user_agent) VALUES (?, ?, ?, ?)").run(
    token,
    userId,
    expiresAt,
    userAgent || null,
...
export function markPushOk(id) {
  db.prepare("UPDATE push_subscriptions SET last_ok_at = ?, last_error = NULL WHERE id = ?").run(
...
export function addCustomRepo(fullName, note = "", userId = 0) {
  try {
    db.prepare("INSERT INTO custom_repos (full_name, user_note, user_id) VALUES (?, ?, ?)").run(
      fullName.trim(),
      note.trim(),
      userId,
    );
...
export function searchCustomRepo(fullName, userId = 0) {
  return db
    .prepare("SELECT * FROM custom_repos WHERE full_name = ? AND user_id = ?")
    .get(fullName.trim(), userId);
}
...
export function hasRecentAlert(repoId, withinHours = 2, kind = null, userId = 0) {
  ...
  if (kind) {
    return (
      db
        .prepare("SELECT id FROM alerts WHERE repo_id = ? AND triggered_at > ? AND kind = ? AND user_id = ?")
        .get(repoId, cutoff, kind, userId) !== undefined
    );
  }
  return (
    db
      .prepare("SELECT id FROM alerts WHERE repo_id = ? AND triggered_at > ? AND user_id = ?")
...
export function insertAlert(
  ...
  growth,
  currentStars,
  { kind = "growth", message = null, userId = 0 } = {},
) {
  db.prepare(
...
export function getApiQuotaInfo() {
  const row = db
    .prepare("SELECT requests_used, requests_limit FROM api_stats ORDER BY id DESC LIMIT 1")
    .get();
...
export function clearRepoStale(fullName) {
  db.prepare("UPDATE repos SET stale = 0, stale_since = NULL, last_error = NULL WHERE full_name = ?").run(
    fullName,
  );
}
...
export function renameRepo(oldName, newName) {
  ...
  try {
    ...
    if (oldRow && newRow && oldRow.id !== newRow.id) {
      ...
      db.prepare(
        "INSERT OR IGNORE INTO favorites (repo_id, added_at) SELECT ?, added_at FROM favorites WHERE repo_id = ?",
      ).run(n, o);
      db.prepare("DELETE FROM favorites WHERE repo_id = ?").run(o);
      db.prepare("UPDATE alerts SET repo_id = ? WHERE repo_id = ?").run(n, o);
      db.prepare(
        "INSERT OR IGNORE INTO query_members (query_id, repo_id) SELECT query_id, ? FROM query_members WHERE repo_id = ?",
      ).run(n, o);
...
export function addTrackedMetric({ source, key, label, url, unit }) {
  try {
    const r = db
      .prepare("INSERT INTO tracked_metrics (source, key, label, url, unit) VALUES (?, ?, ?, ?, ?)")
...
export function insertMetricSnapshot(metricId, value, capturedAt) {
  db.prepare("INSERT INTO metric_snapshots (metric_id, value, captured_at) VALUES (?, ?, ?)").run(
    metricId,
    value,
    capturedAt,
  );
...
export function getMetricSnapshots(metricId) {
  return db
    .prepare("SELECT value, captured_at FROM metric_snapshots WHERE metric_id = ? ORDER BY captured_at ASC")
    .all(metricId);
}
...
export function exportData() {
  return {
    ...
    custom_repos: db
      .prepare("SELECT full_name, user_note, added_at FROM custom_repos ORDER BY user_id, full_name")
      .all(),
    repos: db
      .prepare(
...
export function importData(data, { mode = "merge" } = {}) {
  ...
  try {
    if (mode === "replace") {
      ...
      db.exec(
        "DELETE FROM metric_snapshots; DELETE FROM tracked_metrics; DELETE FROM alerts; DELETE FROM favorites;",
      );
    ...
    const idMap = new Map(
      db
        .prepare("SELECT id, full_name FROM repos")
        .all()
    ...
    const metricMap = new Map(
      db
        .prepare("SELECT id, source, key FROM tracked_metrics")
        .all()
        .map((m) => [`${m.source}\n${m.key}`, m.id]),
    );
    ...
    for (const i of arr(data.indices)) {
      ...
      idxStmt.run(
        i.user_id ?? 0,
        i.name,
        typeof i.spec === "string" ? i.spec : JSON.stringify(i.spec || {}),
        i.created_at ?? null,
      );
      counts.indices++;
    }
    ...
    const queryIdMap = new Map(
      db
        .prepare("SELECT id, user_id, label FROM tracked_queries")
        .all()
        .map((q) => [`${q.user_id}\n${q.label}`, q.id]),
    );
